...
import { timingPresets } from "../../../motion";

import type { AppInputBaseProps } from "./AppInputBase.types";

export const AppInputBase = forwardRef<
...
>(
  (
    ...
    const disabled = editable === false && !readOnly;
    ...
    const animateFocus = useCallback(
      (next: number) => {
        focusOpacity.value = withTiming(next, timingPresets.fast);
      },
      [focusOpacity],
    );
    ...
    return (
      <View
        style={[
          styles.container(size, invalid, disabled, readOnly),
          containerStyle,
        ]}
      >
        ...
        <TextInput
          ...
          onFocus={handleFocus}
          onBlur={handleBlur}
          style={[styles.input(size, disabled), inputStyle]}
          {...rest}
        />
...
AppInputBase.displayName = "AppInputBase";
...
const styles = StyleSheet.create((theme) => ({
  container: (
    ...
    minHeight: theme.sizes.control[size],
    flexDirection: "row",
    alignItems: "center",
  ...
  input: (size: "sm" | "md" | "lg", disabled: boolean) => ({
    minWidth: 0,
    flex: 1,
  ...
  focusRing: {
    ...
    bottom: -1,
    left: -1,
    borderWidth: theme.accessibility.focusRing.width,
